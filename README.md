...
使用 ftp 服务器需要的参数有网址或ip、端口、用户名和密码。在项目中我使用的一个名为  的库，接下来只需三个 java 类即可实现上传、下载、删除。由于在项目中只需上传下载指定文件，所以暂时没有加入选择文件功能，如需要自行加入即可。在代码中该有的注释都有了，方法简洁明了。完整代码已经上至 [Github](https://github.com/fangxiaogang/FtpDome)
